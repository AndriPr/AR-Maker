...
export default function TrashPage() {
  ...
  const handleEmptyTrash = async () => {
    ...
    if (!confirm(`PERINGATAN: Ini akan menghapus permanen ${projects.length} proyek di tong sampah dan tidak bisa dibatalkan. Lanjutkan?`)) return;

    for (const project of projects) {
      if (project.mind_file_url) {
        try {
          const urlParts = project.mind_file_url.split('/public/assets/');
          if (urlParts.length > 1) {
            await supabase.storage.from('assets').remove([urlParts[1]]);
          }
        } catch (e) {
    ...
    await supabase.from('ar_projects').delete().in('id', ids);
    fetchDeletedProjects();
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        ...
        {projects.length > 0 && (
          <button
            ...
            className="shrink-0 flex items-center justify-center gap-2 px-4 py-2 bg-white border border-red-200 text-red-500 font-bold rounded-xl hover:bg-red-50 transition-colors text-sm"
          >
            <Trash2 size={16} /> Kosongkan Sampah
          </button>
        )}
      </div>
